...
class RCModel(object):
    ...
    def _embed(self):
        ...
        self.word_pos_embeddings=tf.get_variable(
		'word_pos_embeddings',	
		shape=(self.vocab_pos.size(),self.vocab_pos.embed_dim),
		initializer=tf.constant_initializer(self.vocab_pos.embeddings),
		trainable=True
		)
                
        self.p_pos_emb=tf.nn.embedding_lookup(self.word_pos_embeddings,self.p_pos)
        self.q_pos_emb=tf.nn.embedding_lookup(self.word_pos_embeddings,self.q_pos)
			
        #concat the word embeddings and word_pos embeddings
	    self.p_emb=tf.concat([self.p_emb,self.p_pos_emb],axis=2)
        self.q_emb=tf.concat([self.q_emb,self.q_pos_emb],axis=2)
    ...
    def _train_epoch(self, train_batches, dropout_keep_prob):
        ...
        for bitx, batch in enumerate(train_batches, 1):
            feed_dict = {self.p: batch['passage_token_ids'],
			 self.p_pos:batch['passage_token_pos_ids'],
                         self.q: batch['question_token_ids'],
			 self.q_pos:batch['question_token_pos_ids'],
