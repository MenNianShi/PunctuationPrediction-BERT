...
def pos_process_batch_data(batch_words,batch_pos, batch_chars, batch_tags=None):
    b_words, b_words_len = pad_sequences(batch_words)
    b_pos,b_words_len = pad_sequences(batch_pos)
    b_chars, b_chars_len = pad_char_sequences(batch_chars)
    if batch_tags is None:
        return {"words": b_words, 'pos':b_pos,"chars": b_chars, "seq_len": b_words_len, "char_seq_len": b_chars_len,
                "batch_size": len(b_words)}
    else:
        b_tags, _ = pad_sequences(batch_tags)
        return {"words": b_words,'pos':b_pos, "chars": b_chars, "tags": b_tags, "seq_len": b_words_len, "char_seq_len": b_chars_len,
                "batch_size": len(b_words)}


def pos_dataset_batch_iter(dataset, batch_size):
    batch_words, batch_chars, batch_tags,batch_pos = [], [], [],[]
    for record in dataset:
        batch_words.append(record["words"])
        batch_chars.append(record["chars"])
        batch_tags.append(record["tags"])
        batch_pos.append(record['pos'])
        if len(batch_words) == batch_size:
            yield process_batch_data(batch_words, batch_pos,batch_chars, batch_tags)
            batch_words, batch_chars, batch_tags ,batch_pos= [], [], [],[]
    if len(batch_words) > 0:
        yield process_batch_data(batch_words,batch_pos, batch_chars, batch_tags)
def bpe_process_batch_data(batch_words, batch_chars, batch_bpe,batch_word_tags,batch_tags=None):
    b_words, b_words_len = pad_sequences(batch_words)
    b_bpe,b_bpe_len = pad_char_sequences(batch_bpe)
    b_chars, b_chars_len = pad_char_sequences(batch_chars)
    if batch_tags is None:
        return {"words": b_words, "chars": b_chars,"bpe":b_bpe,"seq_len": b_words_len, "char_seq_len": b_chars_len,
                "batch_size": len(b_words)}
    else:
        b_tags, _ = pad_sequences(batch_tags)
        b_word_tags,_ = pad_sequences(batch_word_tags)
        return {"words": b_words, "chars": b_chars,"bpe":b_bpe,"word_tags":b_word_tags, "tags": b_tags, "seq_len": b_words_len, "char_seq_len": b_chars_len,
                "batch_size": len(b_words)}
def bpe_dataset_batch_iter(dataset, batch_size):
    batch_words, batch_chars, batch_tags,batch_bpe ,batch_word_tags= [], [], [],[],[]
    for record in dataset:
        batch_words.append(record["words"])
        batch_chars.append(record["chars"])
        batch_tags.append(record["tags"])
        batch_bpe.append(record["bpe"])
        batch_word_tags.append(record["word_tags"])
        if len(batch_words) == batch_size:
            yield bpe_process_batch_data(batch_words, batch_chars,batch_bpe, batch_word_tags,batch_tags)
            batch_words, batch_chars, batch_tags,batch_bpe,batch_word_tags= [], [], [],[],[]
    if len(batch_words) > 0:
        yield bpe_process_batch_data(batch_words, batch_chars,batch_bpe,batch_word_tags, batch_tags)
